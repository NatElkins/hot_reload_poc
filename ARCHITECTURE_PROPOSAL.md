...
| `SymbolChanges` (src/Compilers/Core/Portable/Emit/EditAndContinue/SymbolChanges.cs) | Aggregates semantic edit classification, synthesised member tracking, and deleted member sets. | `FSharpSymbolChanges` (planned, alongside `FSharpSymbolMatcher`) | Build on `FSharpDefinitionMap` + `HotReloadNameMap` to enumerate changed entities, synthesised closures/state machines, and deleted members for `IlxDeltaEmitter`. |
