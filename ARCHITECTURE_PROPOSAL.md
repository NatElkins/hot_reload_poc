...
- **Delta stream scaffolding**: `IlxDeltaStreamBuilder` (`src/Compiler/CodeGen/IlxDeltaStreams.fs`) now mirrors Roslyn’s metadata builder pipeline for method-body-only edits. It captures encoded method bodies, materialises EncLog/EncMap entries, records standalone-signature blobs via `AddStandaloneSignature`, and emits non-empty metadata/IL blobs. `IlxDeltaEmitter` re-emits the updated `ILModuleDef` via `ILBinaryWriter`, copies the relevant `MethodDef` rows (including updated RVAs) into the delta stream, and the component tests validate the result with `mdv` as well as a `MetadataReader` sanity check.
