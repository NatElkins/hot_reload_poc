...
- **Delta stream scaffolding**: `IlxDeltaStreamBuilder` (`src/Compiler/CodeGen/IlxDeltaStreams.fs`) now mirrors Roslyn’s metadata builder pipeline for method-body-only edits. It captures encoded method bodies, materialises preliminary EncLog/EncMap entries, records standalone-signature blobs via `AddStandaloneSignature`, and emits non-empty metadata/IL blobs. `IlxDeltaEmitter` re-emits the updated `ILModuleDef` via `ILBinaryWriter` to hydrate the builder, producing deltas that already validate with `mdv` in component tests.
