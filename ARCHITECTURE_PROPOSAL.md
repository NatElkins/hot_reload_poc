...
| Metadata writer row emission | `tests/FSharp.Compiler.Service.Tests/HotReload/FSharpDeltaMetadataWriterTests.fs` | Service-level regression suite that constructs IL helpers in-memory, runs `FSharpDeltaMetadataWriter.emit`, and asserts Property/PropertyMap EncLog entries plus table counts match Roslyn expectations before the heavier component harness runs. |
