...
| Portable PDB deltas | `tests/FSharp.Compiler.ComponentTests/HotReload/PdbTests.fs` | Covers method, property, event accessor, and now multi-generation method edits; asserts each portable PDB delta references the correct MethodDef token and documents so sequence points remain valid across generations. |
