...
- **Delta stream scaffolding**: `IlxDeltaStreamBuilder` (`src/Compiler/CodeGen/IlxDeltaStreams.fs`) now mirrors Roslyn’s metadata builder pipeline for method-body-only edits. It captures encoded method bodies, materialises preliminary EncLog/EncMap entries, records standalone-signature blobs via `AddStandaloneSignature`, and emits non-empty metadata/IL blobs (with placeholder RVAs) so the upcoming `IlxDeltaEmitter` integration can swap out today’s placeholder payloads.
