...
| `DeltaMetadataWriter` (src/Compilers/Core/Portable/Emit/EditAndContinue/DeltaMetadataWriter.cs) | Emits ECMA-335 table slices, EncLog/EncMap entries, and metadata heaps for each generation. | `IlxDeltaEmitter` + forthcoming `FSharpDeltaMetadataWriter` | Mirror Roslyn’s pipeline: build per-table “definition indexes” for changed symbols, serialize only appended rows using baseline heap/table offsets from `MetadataSnapshot`, generate EncLog/EncMap in one place, and update `FSharpEmitBaseline` with cumulative table/heap growth, synthesized/deleted members, and per-method delta info. |
...
- **Metadata delta writer parity**: Roslyn’s `DeltaMetadataWriter` layers definition indexes over each metadata table, reuses baseline heap offsets from `EmitBaseline`, and returns an updated baseline via `EmitBaseline.With(...)`. F# will mirror this design by introducing `FSharpDeltaMetadataWriter`, extending `FSharpEmitBaseline` with cumulative table/heap growth and synthesized/deleted member maps, and centralising EncLog/EncMap generation so `IlxDeltaEmitter` no longer depends on a full `MetadataBuilder` re-emit.
