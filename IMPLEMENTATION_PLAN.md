...
- **Next steps (execute in order)**:
  ...
  3. **Rewrite `FSharpDeltaMetadataWriter` to append table rows** – ⚙️ *in progress*: Roslyn copies each updated `MethodDefinition` from the baseline reader, swaps in the new RVA/local sig token, and appends the row + EncLog entry. We need to do the same:
     - emit MethodDef rows with updated RVA/locals signature, using `metadataReader.GetMethodDefinition` to lift attributes/signature blobs, and use the definition indexes above to decide whether the row is “updated” vs “added”.
     - emit Param rows (and, once symbol data is available, EventMap/PropertyMap slices) when the definition index reports additions, mirroring `DeltaMetadataWriter.Populate*TableRows`.
     - build EncLog/EncMap entries from the definition indexes (Roslyn’s `PopulateEncLogTableRows`/`PopulateEncMapTableRows`), and return the updated Enc IDs + per-table counts so `HotReloadBaseline.applyDelta` can persist them.
     - *Update 2025-11-07*: MethodDef + Param rows now flow through the new writer. `IlxDeltaEmitter` captures parameter row order per method, the metadata writer appends those rows (along with Module/Method EncLog entries), and the returned table counts feed directly into `HotReloadBaseline.applyDelta`. Event/Property(+map) rows remain TODO so keep this subtask open until the rest of the tables are covered.
     - *Update 2025-11-07*: `FSharpDeltaMetadataWriter` now owns the metadata builder instance used during IL rewriting, consumes the captured `MethodDefinitionRowInfo` list, and serializes real MethodDef rows plus EncLog/EncMap entries. `IlxDeltaEmitter` no longer relies on `IlxDeltaStreamBuilder` for metadata, and the regression suite (`IlDeltaStreamBuilder emits aligned method bodies`, `emitDelta records method definition rows`) verifies the new ordering. Param/Event/Property rows remain TODO; keep this subtask open until those tables are also emitted.
  4. **Emitter integration** – refactor `IlxDeltaEmitter.emitDelta` to feed the richer writer:
  ...
  6. **Event/Property metadata parity** – Remaining steps before we can emit the Event/Property tables:
    ...
    - *Update 2025-11-07*: DeltaBuilder now groups property/event accessor symbols, resolves baseline method handles where available, and surfaces them via `UpdatedAccessors` on `DeltaEmissionRequest`/`IlxDeltaRequest`. IlxDeltaEmitter can now inspect accessor metadata when emitting MethodSemantics/property rows.
     ...
     - Follow-up: re-implement the removed `IlDeltaStreamBuilder` component coverage using the new `IlxDeltaStreamBuilder` APIs so metadata/IL stream building continues to be validated.
  6. **PDB alignment** – update `HotReloadPdb.emitDelta` to consume the enriched baseline (`AddedOrChangedMethodInfo`, Enc IDs, synthesized member map) when it emits sequence points. Roslyn reference: `EditAndContinueMethodDebugInformationWriter` and `PortablePdbBuilder` in `MetadataWriter.cs`.
  7. **Tests & validation** – once rows are fully emitted:
  ...
  8. **Status (2025-11-05)** – Roslyn parity work in progress. mdv component tests remain red because the new writer is not implemented; keep this item open until the new infrastructure ships.
