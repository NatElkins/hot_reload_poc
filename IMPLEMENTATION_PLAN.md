...
- **Subtask tracker** (each with its own commit/tests report):
  ...
  - ⏳ **Broaden test coverage**: Layer in additional scenarios (async state machines, closures, computation expressions, multi-generation) once the delta output is stable. Added initial IL-byte assertions for method-body edits (2025-10-31) and verified emitted MethodDef rows via `MetadataReader`; broader scenarios remain.
