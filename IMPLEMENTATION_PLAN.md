...
- **Subtask tracker** (each with its own commit/tests report):
  ...
  - ✅ **Drive multi-generation CLI flow** (2025-10-31): `HotReloadTest` now reuses the same generator across sequential edits, emits generation-numbered delta artifacts, and reruns mdv before each `MetadataUpdater.ApplyUpdate`. Validation: `dotnet build hot_reload_poc/HotReloadPoc.sln -c Debug`.
  ...
  - ⏳ **Broaden test coverage**: Layer in additional scenarios (async state machines, closures, computation expressions, multi-generation) once the delta output is stable. Added initial IL-byte assertions for method-body edits (2025-10-31) and verified emitted MethodDef rows via `MetadataReader`/`mdv`; broader scenarios remain.
...
- **Follow-up**: Surface the new `HotReloadState` session hooks through forthcoming orchestrators (CLI/IDE) so baseline/EncId updates are triggered automatically after each applied delta.
