...
- **Next steps (execute in order)**:
  ...
  2. **Populate definition indexes with real rows** – the helper plumbing is in place, but today we never enqueue new rows. Mirroring Roslyn’s `DefinitionIndex<T>` means:
     ...
     - *Update 2025-11-09 (pm)*: Portable PDB coverage now spans accessor additions. `HotReload.PdbTests` include property/event scenarios that start from a host baseline (no accessor) and verify the returned PDB delta carries documents + sequence points for the newly added accessor tokens.
