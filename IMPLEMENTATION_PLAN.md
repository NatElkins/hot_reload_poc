...
- **Context**: Roslyn `DeltaMetadataWriter`, `EmitBaseline.With(...)`, `SymbolChanges`, and `SynthesizedTypeMaps`.
      - **Status 2025-11-07**: Helper-driven component tests (`tests/FSharp.Compiler.ComponentTests/HotReload/MdvValidationTests.fs`, `.../PdbTests.fs`) now exercise property/event accessor edits without going through msbuild. mdv asserts that accessor deltas log the expected method tokens, and the PDB suite confirms the portable PDB delta references those tokens.
      - **Status 2025-11-11**: Fixed the missing inverse token remap in `HotReloadPdb.emitDelta`; property/event accessor PDB tests are green again.
      - **Status 2025-11-12**: `MdvValidationTests` gained the multi-generation property helper; both metadata blobs and EncLog slices now match the Roslyn reference dumps across Gen1/Gen2.
      - **Status 2025-11-12 (evening)**: `DeltaMetadataTables` uses dedicated row + heap builders (no `ILBinaryWriter` tables), and `DeltaMetadataSerializer` no longer imports `ILBinaryWriter`.
      - **Status 2025-11-12 (late night)**: Added `MetadataDeltaTestHelpers.fs` + `FSharpMetadataAggregatorTests.fs` so the aggregator path is validated end-to-end (baseline + delta readers, method + string handle translation).
      - **Status 2025-11-13**: Aggregator tests now also verify `TranslateStringHandle`, ensuring string literals remap to the baseline heap before we expand to multi-generation scenarios.
      - **Status 2025-11-13 (mid-day)**: Introduced `DeltaMetadataSizes` in `DeltaMetadataSerializer.fs` and threaded it through `FSharpDeltaMetadataWriter.fs`, so row/heap counts, bit masks, and coded-index flags travel together (mirroring Roslyn’s `MetadataSizes`). Next sub-step is to update `DeltaIndexSizing.fs`/`DeltaTableLayout.fs` to consume the new record directly.
      - **Status 2025-11-11 (late afternoon)**: Re-opened `FSharp.Compiler.AbstractIL.ILBinaryWriter`/`BinaryConstants` inside `DeltaMetadataTables.fs` and `DeltaMetadataSerializer.fs` so `RowElementTags`, `TableNames`, and `tdor_*` helpers remain available on netstandard2.0 while the refactor is still in flight. With the imports restored, the service HotReload filter builds again, but `FSharpMetadataAggregatorTests` continue to fail—the string-handle case returns `"SampleModule"` because we currently re-emit method names into a fresh delta string heap at offset `0x01` instead of reusing the baseline handle for `"get_Message"`. Upcoming work needs to capture baseline `StringHandle`/`BlobHandle` data (or a lookup table) alongside the row info so unchanged literals keep their original heap offsets before we exercise `MetadataAggregator.TranslateStringHandle`.
      - **Micro plan for heap/handle parity (Option 1)**:
        1. **Surface baseline heap offsets** – Thread `MetadataSnapshot.HeapSizes` (string/blob/user-string/guid) through `IlxDeltaEmitter` into the row mirrors so `DeltaMetadataTables` knows each heap’s starting offset (Roslyn passes these to `MetadataBuilder` via `MakeTablesBuilder`).
        2. **Extend row info with baseline handles** – Teach `MethodDefinitionRowInfo`, `PropertyDefinitionRowInfo`, etc., to carry the original `StringHandle`/`BlobHandle` from the baseline reader alongside the textual data we already capture. For updated members we should prefer the baseline handle; for newly added members we’ll fall back to a new literal.
           - **Status 2025-11-11 (evening)**: `IlxDeltaEmitter` and the metadata-writer tests now capture the baseline `StringHandle`/`BlobHandle` for every method/parameter/property/event row. We’re not reusing them yet (the SRM builder still emits fresh handles), but the data is available for the AbstractIL serializer once we finish Step 3.
        3. **Augment heap builders** – Update `StringHeapBuilder`/`ByteArrayHeapBuilder` inside `DeltaMetadataTables` so they can either (a) return an existing absolute heap offset (when a baseline handle is available) without duplicating bytes, or (b) append new bytes and add the baseline start offset to the returned index (matching `MetadataBuilder`’s `stringHeapStartOffset` behavior in Roslyn).
           - **Status 2025-11-12 (late evening)**: `DeltaMetadataTables` now consumes the captured `NameHandle`/`SignatureHandle` values for methods, parameters, properties, and events, invoking the shared `AddExistingEntry` paths so baseline offsets are retained. `FSharpMetadataAggregatorTests` now pass again, proving reused literals resolve to generation 0 handles before we trim the heaps further.
           - **Status 2025-11-13 (late)**: The first pass at reusing absolute offsets proved invalid—`MetadataReader` cannot decode deltas whose string/blob handles point outside the local heaps (aggregator tests started throwing `BadImageFormatException`). We reverted to copying the baseline prefix back into the delta heaps and will revisit handle reuse once we understand how Roslyn keeps its heaps minimal without breaking standalone readers.
          - **Status 2025-11-13 (pm)**: Added a dedicated user-string heap builder plus wiring in `FSharpDeltaMetadataWriter`/`IlxDeltaEmitter` so `UserStringUpdates` populate the `#US` stream. The mdv helper/component suites now find the updated literals in generation 1 deltas, eliminating the stale `"Message version 3"` output and unblocking runtime ApplyUpdate investigations.
          - **Status 2025-11-12 (late night +4)**: `FSharpDeltaMetadataWriter.emit` now accepts full `(originalToken * newToken * literal)` triples, and `IlxDeltaEmitter` threads the original user-string token through unchanged so the mirror can remap offsets directly to the baseline heaps. The service/component writer tests still compile against the pre-change signature (they reference the netstandard2.0 ref assembly), so updating `FSharpDeltaMetadataWriterTests.fs`/`MetadataDeltaTestHelpers.fs` to pass the triple and regenerating the ref assembly is the next unblocker.
          - **Status 2025-11-12 (late night +5)**: Added an `emitWithUserStrings` helper plus a legacy `emit` overload that supplies an empty user-string list, keeping the public surface compatible with the netstandard2.0 reference assembly. The hot-reload service tests now build/run against net10.0 without touching their `DeltaWriter.emit` call sites, so we can stage the remaining writer changes incrementally.
      - **Status 2025-11-12 (late night +6)**: Added regression tests in `FSharpDeltaMetadataWriterTests` that assert the writer sets ENC-friendly `IndexSizes` (all heaps/coded indexes promoted to "big") and that the table bitmasks only flag tables with actual rows, matching the expected Roslyn semantics for property/event map deltas.
      - **Parity requirements (standing rule)**: when implementing or refactoring any part of the writer, diff the equivalent Roslyn sources (`MetadataWriter.cs`, `MetadataSizes.cs`, `ApplyDelta.cs`) and ensure: (1) the same inputs (`MetadataSizes` record) flow through heap builders, table bitmasks, and serializer; (2) baseline handles/heap offsets are reused verbatim (no duplicate literals); (3) mdv *and* `MetadataUpdater.ApplyUpdate` succeed for the demo edit before marking a step complete. No shortcuts—each sub-step is “done” only when both parity checks pass.
          - **Status 2025-11-12 (late night +7)**: Refactored `DeltaIndexSizing.compute` to consume the mirror’s `MetadataHeapSizes` (instead of raw heap lengths) and threaded the new signature through `DeltaMetadataSerializer`/`FSharpDeltaMetadataWriter`. This keeps the sizing helper on the same abstraction boundary as `DeltaTableLayout` and matches Roslyn’s `MetadataSizes` inputs.
          - **Status 2025-11-12 (late night +8)**: `RowElementData`/`DeltaMetadataTables`/`DeltaMetadataSerializer` now tag reused string/blob handles as absolute heap offsets, so the serializer bypasses the delta-local lookup and emits the baseline offset directly (matching Roslyn’s `MetadataWriter`). This is the final prerequisite before validating mdv + runtime apply on the demo edit.
        4. **Serializer updates** – Teach `DeltaMetadataSerializer.writeRowElement` to respect the new “absolute offset” entries (i.e., skip heap-index lookups when a row already carries a precomputed offset) and ensure the emitted heap blobs include the correct zero-padding so mdv/MetadataReader can resolve handles ≥ baseline size.
        5. **Regression coverage** – Extend `FSharpDeltaMetadataWriterTests` and `FSharpMetadataAggregatorTests` so they assert: (a) method/property name handles for pure body edits equal the baseline handle, (b) the string heap length stays zero for unchanged literals, and (c) aggregator translation returns the baseline string (`"get_Message"`) rather than `"SampleModule"`.
        - **Status 2025-11-11 (afternoon)**: `IlxDeltaEmitter` now derives `MetadataHeapOffsets` from the baseline snapshot and threads them into `DeltaMetadataTables`/`FSharpDeltaMetadataWriter`. The tests still fail in `FSharpMetadataAggregatorTests` (expected) because we haven’t yet plumbed the baseline handles or taught the serializer to reuse heap slots, but the new plumbing unblocks Step 2.

  - **Detailed next actions (2025-11-13)**
    1. **Refactor `DeltaIndexSizing`/`DeltaTableLayout` (Roslyn parity)**
       - Files: `src/Compiler/CodeGen/DeltaIndexSizing.fs`, `DeltaTableLayout.fs`, `DeltaMetadataSerializer.fs`, `tests/FSharp.Compiler.Service.Tests/HotReload/FSharpDeltaMetadataWriterTests.fs`.
       - Steps:
         - Mirror Roslyn’s `MetadataSizes` (`roslyn/src/Compilers/Core/Portable/PEWriter/MetadataSizes.cs`) by introducing an internal record that carries row counts, heap sizes, and bitmasks from `DeltaMetadataTables`.
         - Replace any remaining references to `ILBinaryWriter.MetadataTable<'T>` sizing helpers with computations derived from the mirror’s `TableRowCounts`/`HeapSizes`.
         - Add focused tests that inspect `metadataDelta.IndexSizes`/`TableBitMasks` to ensure property/event map scenarios flip the same coded-index flags Roslyn expects.
       - Acceptance: `FSharpDeltaMetadataWriterTests` keep passing on netstandard2.0/net10.0, and HotReload service/component filters stay green.

    2. **Broaden metadata/aggregator multi-generation coverage**
       - Files: `tests/FSharp.Compiler.ComponentTests/HotReload/MdvValidationTests.fs`, `tests/FSharp.Compiler.ComponentTests/HotReload/PdbTests.fs`, `tests/FSharp.Compiler.Service.Tests/HotReload/FSharpMetadataAggregatorTests.fs`, helper updates in `MetadataDeltaTestHelpers.fs`.
       - Steps:
         - Extend the helper to emit a second property delta (Generation 2) plus an event delta, mirroring Roslyn’s `MetadataAggregatorTests.MembersAcrossGenerations` coverage.
         - Add mdv component tests that drive property/event edits across two generations and assert EncLog/EncMap + mdv output match Roslyn dumps (`AddProperty`, `UpdateMethod`, etc.).
         - Teach the aggregator tests to translate method + string handles for baseline/Gen1/Gen2 so `TranslateHandle` is proven once multiple deltas exist.
       - Acceptance: New tests fail prior to the fixes and pass afterwards; component/service HotReload filters remain green.

    3. **Runtime demo validation (`HotReloadDemoApp`)**
       - Files: `tests/projects/HotReloadDemo/HotReloadDemoApp`, `tests/scripts/hot-reload-demo-smoke.sh`, documentation in `docs/HotReload.md`.
       - Steps:
         - Wire the CLI sample to capture emitted metadata/IL/PDB blobs, call `MetadataUpdater.ApplyUpdate`, and print both the mdv command and the observed literal before/after applying the delta.
         - Update the scripted smoke test to run in `--scripted` mode, patch the sample file once, call `MetadataUpdater.ApplyUpdate`, and assert the console output shows the updated literal without restarting the process. Record the mdv paths for manual inspection.
         - Document the workflow (env vars, expected console markers, troubleshooting tips) and cross-link from `ARCHITECTURE_PROPOSAL.md` so anyone can rerun the runtime scenario.
       - Acceptance: `tests/scripts/hot-reload-demo-smoke.sh` passes locally/CI, and the documentation includes the new instructions + diagnostics.
- **Next steps (execute in order)**:
  ...
  10. **New action (2025-11-11)** – Capture the AbstractIL table snapshots as plain `RowElementData[][]` structures inside `DeltaMetadataTables` so module-scope `TableRows` no longer depend on `ILBinaryWriter.MetadataTable<'T>`. Once that mirror exists we can reapply the FS0058 fix without fighting internal-type visibility.
      - **Status 2025-11-12**: `DeltaMetadataTables` now uses an internal `RowTableBuilder` that stores `RowElementData[]` directly; module/method/param/property/event/map/method-semantics rows no longer instantiate `MetadataTable<UnsharedRow>`. The string/blob/guid heaps still rely on `MetadataTable<'T>` for deduplication, so the next leg is to introduce lightweight shared-heap builders (string/blob/guid) before we delete the remaining AbstractIL dependencies.
      - **Status 2025-11-12 (evening)**: Added dedicated string/blob/guid heap builders backed by simple dictionaries so the metadata mirror no longer depends on `MetadataTable<'T>` at all. The next action is to thread these new heap snapshots through `DeltaMetadataSerializer` so heap padding/length calculations rely solely on the mirror instead of the AbstractIL writer.
      - **Status 2025-11-12 (late)**: Removed the lingering `ILBinaryWriter` imports from both `DeltaMetadataTables.fs` and `DeltaMetadataSerializer.fs`; the serializer already reads bytes from the mirror, so the only remaining AbstractIL usage in this area is inside `DeltaIndexSizing`/`DeltaTableLayout`. Next up: update those helpers (and the serializer) to rely on the mirror's heap/table counts exclusively so we can delete the remaining IL-specific types.
      - **Status 2025-11-12 (night)**: Checked in `MetadataDeltaTestHelpers.fs` plus `FSharpMetadataAggregatorTests.fs`, which now validate the `FSharpMetadataAggregator` wrapper end-to-end (baseline + delta readers, method/string handle translation). The helpers emit real metadata deltas via the new writer, so future Aggregator/API work can build on a stable testbed.
      - **Status 2025-11-12 (late night)**: Expanded the aggregator suite with a string-handle regression to ensure `TranslateStringHandle` remaps delta handles back to the baseline string heap. This protects the `FSharpMetadataAggregator` wrapper as we begin threading it through more of the hot-reload pipeline.
      - **Status 2025-11-12 (late night +1)**: Delta table serializer now derives the heap-size flags from the computed `CodedIndexSizes` (instead of hard-coding `0x07`), but `FSharpMetadataAggregatorTests` still fail with `System.BadImageFormatException (Read out of bounds)`. Next up: align the mirrored heap bytes with the reused baseline handles so `MetadataReader` can parse the emitted delta before aggregation.
      - **Status 2025-11-12 (late night +2)**: String/blob heap builders now mirror baseline handles by copying the literal bytes back into the delta heaps, and the serializer sets the Enc delta flags automatically (`StringHeapLarge|GuidHeapLarge|BlobHeapLarge|EncDeltas|DeletedMarks`). `FSharpMetadataAggregatorTests` pass end-to-end, and `FSHARP_HOTRELOAD_TRACE_METADATA=1` now dumps the custom vs. SRM metadata so future regressions can be diagnosed quickly.
      - **Status 2025-11-12 (late night +3)**: The metadata writer no longer emits Method/Param/Property/Event(+map) rows into `System.Reflection.Metadata.MetadataBuilder` by default (guarded behind `FSHARP_HOTRELOAD_USE_SRM_TABLES=1`). We still feed the SRM builder the module/EncLog entries, but all table bytes now come exclusively from the AbstractIL mirror + serializer, completing Step 3’s “move off MetadataBuilder” goal.
      - **Status 2025-11-13**: Added `FSharpMetadataAggregatorTests``multi-generation string handle` scenario to ensure string handles translate back to baseline even after multiple deltas. The helper now supports custom literals so we can diff successive generations (“generation-one” → “generation-two”) in a single test.
     - **Status 2025-11-13 (pm)**: Added the companion event scenario (`aggregator translates event method handles across generations`) plus helper support for emitting event deltas with custom literals. This extends Step 4 coverage beyond properties so MethodSemantics/event-map paths now prove their handles remap to generation 0 as well.
     - **Status 2025-11-13 (late pm)**: Parameter definition metadata now preserves the baseline `Param` row ids and the hot-reload emitter rewrites existing methods to point at those absolute rows. `FSharpMetadataAggregator` gained parameter-handle/string translation plus new service tests that lock the behavior in; reran `./.dotnet/dotnet test tests/FSharp.Compiler.Service.Tests/FSharp.Compiler.Service.Tests.fsproj -c Debug -f net10.0 --filter FullyQualifiedName~FSharpMetadataAggregatorTests` after fixing the FS3218 noise and the suite now passes.
     - **Status 2025-11-13 (late night)**: The metadata delta helpers now pass real heap offsets for both property and event scenarios, so mdv dumps and Runtime ApplyUpdate share the same string/blob/guid bases as Roslyn. The aggregator suite gained parameter-handle and parameter-name coverage to catch future regressions in Param table/heap reuse.
     - **Status 2025-11-13 (night +1)**: `FSharpDeltaMetadataWriter` now routes even the empty-delta path through `DeltaMetadataSerializer.computeMetadataSizes`, deleting the bespoke `DeltaIndexSizing`/`DeltaTableLayout` calls. This keeps the codepaths uniform and sets us up to pass the shared `DeltaMetadataSizes` record deeper into the serializer.
     - **Status 2025-11-13 (night +2)**: `DeltaMetadataSerializer.serializeMetadataRoot` now consults `DeltaMetadataSizes.IsEncDelta` so we only emit the `#JTD` stream when the delta actually contains ENC tables, matching Roslyn’s layout and keeping non-ENC blobs leaner.
     - **Status 2025-11-13 (night +3)**: `FSharpDeltaMetadataWriterTests` gained stream-name coverage so we prove the new behavior: the empty-delta sample contains only `#-, #Strings, #US, #GUID, #Blob`, while a property edit delta reintroduces `#JTD`. This locks in the stream layout before we tackle the remaining serializer refactors.
     - **Status 2025-11-12 (late pm)**: Updated the `HotReloadDemo` sample so its scripted mode now runs with `--runtime-apply` by default (`DOTNET_MODIFIABLE_ASSEMBLIES=debug` + `FSHARP_HOTRELOAD_ENABLE_RUNTIME_APPLY=1`) and the baseline literal tracks `generation 0`. `MetadataUpdater.ApplyUpdate` still throws `InvalidOperationException("The assembly update failed.")`, so the scripted run fails after emitting the first delta; keep digging into CLR diagnostics before flipping the smoke-test gate on in CI.
     - **Status 2025-11-12 (night)**: Captured baseline string/blob handles when starting the session (`HotReloadBaseline.attachMetadataHandles`) and plumbed them through `IlxDeltaEmitter`/`FSharpDeltaMetadataWriter`, so mdv now shows Generation 1 metadata preserving the original module/method names (e.g., `GetMessage` still points at offset `0x2F0`). The runtime demo still fails with the opaque `MetadataUpdater.ApplyUpdate failed: The assembly update failed.` message even though the metadata looks sane. **Next actions**:
       - Diff our emitted metadata/IL/PDB delta against Roslyn’s for the same edit (string literal update) to see which heaps or EncLog entries still diverge (especially user-string handling and the `#JTD` stream).
       - Add targeted tracing around the user-string remapper so we can prove whether new literals are being emitted as fresh `#US` entries instead of reusing baseline tokens.
       - Investigate the CLR ApplyUpdate logging knobs (EventPipe provider or `COMPlus_ApplyUpdate*` env vars) so we can capture the precise failure reason before iterating on the writer again.
     - **Status 2025-11-12 (late night +4)**: Generated Roslyn deltas via `hotreload-utils`/`csharp_delta_test` and compared them to the F# demo deltas. The C# metadata heaps stay tiny (e.g., `#Strings` = 0x3d bytes, `#US` = 4 bytes, EncLog includes AssemblyRef + TypeRef rows), whereas the F# delta still dumps a ~0x3B0-byte string heap full of empty entries and only logs Module/Method updates. This confirms we’re over-emitting baseline strings and probably confusing the CLR before the aggregator runs. Next up: shrink our heap emission to only the rows/handles touched by the delta and make sure we log the same EncLog rows Roslyn does.
      - **Status 2025-11-12 (night +5)**: Updated `FSharpMetadataAggregator.TranslateStringHandle` to fall back to baseline handles by looking up the literal value in the baseline metadata (method/property/event/type names). This keeps existing tests passing even if the delta re-encodes names at new offsets, which unblocks the next iteration where we remap handles without cloning the entire baseline string blob.
