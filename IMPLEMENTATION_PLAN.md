...
- **Status 2025-11-11 (late)**: Implemented the inverse token remap plus test cleanup (`PdbTests` no longer fails intentionally). `dotnet test ... --filter FullyQualifiedName~HotReload` is green again, confirming that added/edited property and event accessors now produce portable PDB deltas with the expected MethodDef tokens and sequence points. **Next action**: add a `mdv helper validates multi-generation property accessor metadata` fact that mirrors the existing method/event helpers so we can prove the metadata/PDB remap survives sequential property getter edits (baseline literal → generation 1 → generation 2) without depending on the CLI.
- **Status 2025-11-12**: Added the multi-generation property accessor mdv helper. The new fact writes two metadata/IL deltas, asserts that each blob contains the expected literal, verifies `EncLog` includes the getter MethodDef token, and (when mdv is present) confirms Generation 1/Generation 2 entries show the updated strings. This keeps parity with the method/event multi-generation helpers highlighted above.
