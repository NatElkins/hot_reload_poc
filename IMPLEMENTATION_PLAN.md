  ...
  3. **Rewrite `FSharpDeltaMetadataWriter` to append table rows** – ⚙️ *in progress*: Roslyn’s `DeltaMetadataWriter` (`roslyn/src/Compilers/Core/Portable/Emit/EditAndContinue/DeltaMetadataWriter.cs`) copies each updated method from the baseline reader, swaps in the new RVA/local-signature token, and appends MethodDef/Param/Event/Property rows plus EncLog entries. To reach parity:
     ...
     - **Upcoming (new breakdown, 2025-11-09)**: To finish the Roslyn parity work we need a thin AbstractIL serializer that emits the `#~` stream using `DeltaMetadataTables.TableRows` + heap blobs (no SRM involvement). Break this into the following substeps:
       ...
       5. *Integration & dual-path tests*: teach `FSharpDeltaMetadataWriter` to call the new serializer (behind a feature flag). Extend `FSharpDeltaMetadataWriterTests` to assert that the AbstractIL-serialized metadata matches the current SRM output byte-for-byte before making it the default. Once validated, remove the SRM dependency entirely so future deltas are emitted via the AbstractIL writer.
          - *Update 2025-11-09 (late pm)*: `FSharpDeltaMetadataWriter` now emits the AbstractIL `#~` stream snapshot on every delta, and `FSharpDeltaMetadataWriterTests.fs` extracts the SRM `#~` stream to assert byte-for-byte parity for the property/event scenarios. The serializer is still behind `FSHARP_HOTRELOAD_USE_ABSTRACTIL`; the remaining work is to route the AbstractIL stream into the final metadata blob and flip the guard once larger scenarios (method/closure/async) pass.
          - *Update 2025-11-09 (late pm)*: `DeltaMetadataSerializer` now assembles the entire metadata root (signature, version header, stream headers, #~ + heap streams) when `FSHARP_HOTRELOAD_USE_ABSTRACTIL=1`. `FSharpDeltaMetadataWriter` swaps in that blob when the flag is set, while continuing to produce SRM metadata by default. **Next step**: expand the parity tests to cover larger method/closure/async scenarios and, once they match, flip the guard so the AbstractIL serializer becomes the default path (removing the SRM dependency).
          - *Update 2025-11-10*: Added `FSharpDeltaMetadataWriterTests` coverage for single-parameter methods, closure-like helpers, and async/state-machine pairs. Each scenario toggles `FSHARP_HOTRELOAD_USE_ABSTRACTIL` and asserts that the AbstractIL metadata blob (and table stream) matches the SRM baseline byte-for-byte while still checking the expected MethodDef/Param row counts. Guard remains in place until we can run the failing compiler build.
...
- **Micro-plan for metadata-row refactor (2025-11-11)**:
  ...
  5. 🟡 **In progress** – Short-circuit SRM serialization when the AbstractIL path is enabled. Step 1 complete: `FSharpDeltaMetadataWriter` now skips `MetadataRootBuilder.Serialize` when `FSHARP_HOTRELOAD_USE_ABSTRACTIL` is set, relying entirely on `DeltaMetadataSerializer` for metadata bytes. Follow-up: drop the SRM builder entirely once the guard flips.
