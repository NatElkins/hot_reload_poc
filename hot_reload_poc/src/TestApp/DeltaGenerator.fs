...
type DeltaGenerator = {
    ...
    /// <summary>The generation number to emit next (1-based, increments after each successful delta).</summary>
    CurrentGeneration: int
    /// <summary>The EncId produced by the most recent delta (None until the first delta is emitted).</summary>
    PreviousEncId: Guid option
}
...
module DeltaGenerator =
    ...
    /// <summary>
    /// Generates metadata delta for a method update, handling EncBaseId based on generation and returning the new EncId.
    /// </summary>
    ...
    /// Returns the updated generator state together with the delta (if one was produced).
    /// </summary>
    let generateDelta (generator: DeltaGenerator) (assembly: Assembly) (returnValue: int) (isInvokeStub: bool) (typeName: string) (methodName: string) =
        let generation = generator.CurrentGeneration
        let previousEncId = generator.PreviousEncId
        ...
        async {
            ...
            | method' ->
                ...
                let updatedGenerator = {
                    generator with
                        PreviousReturnValue = Some returnValue
                        CurrentGeneration = generation + 1
                        PreviousEncId = Some nextEncId
                }

                return updatedGenerator, Some {
                    ModuleId = moduleId
