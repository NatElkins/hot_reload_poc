    ...
    let runTest () =
        async {
            ...
            | Some (_, (typeName, methodName), compiledBaselinePath) ->
                ...
                let applyDelta targetValue expectedValue =
                    async {
                        printfn "[HotReloadTest] Generating delta to change return value to %d..." targetValue
                        let! nextGenerator, delta =
                            DeltaGenerator.generateDelta generator originalAssembly targetValue isInvokeStub typeName methodName

                        generator <- nextGenerator

                        match delta with
                        | None ->
                            printfn "[HotReloadTest] Failed to generate delta for target value %d" targetValue
                            return false
                        | Some delta ->
                            let generationNumber = nextGenerator.CurrentGeneration - 1
                            printfn "[HotReloadTest] Generated delta (generation %d):" generationNumber
                            ...
                            printfn "[HotReloadTest] Delta details (generation %d):" generationNumber
                            ...
                            let metaFile = sprintf "%d.meta" generationNumber
                            let ilFile = sprintf "%d.il" generationNumber
                            let pdbFile = sprintf "%d.pdb" generationNumber

                            printfn "[HotReloadTest] Writing delta files (%s, %s, %s) to %s" metaFile ilFile pdbFile deltaDir
                            File.WriteAllBytes(Path.Combine(deltaDir, metaFile), metadataBytes)
                            File.WriteAllBytes(Path.Combine(deltaDir, ilFile), ilBytes)
                            File.WriteAllBytes(Path.Combine(deltaDir, pdbFile), pdbBytes)
                            ...
                            try
                                let fsMetaPath = Path.Combine(deltaDir, metaFile)
                                ...
                                let csMetaBase = "/Users/nat/Projects/ExpressionEvaluator/csharp_delta_test/bin/Debug/net10.0"
                                let csMetaPath = Path.Combine(csMetaBase, metaFile)
                            ...
                            let scriptPath = Path.Combine(Path.GetTempPath(), $"run_mdv_gen{generationNumber}.sh")
                            let scriptContent =
                                "#!/bin/bash\n\n" +
                                "# Change to the specified directory\n" +
                                $"cd \"{deltaDir}\" || {{ echo \"Failed to change directory\"; exit 1; }}\n\n" +
                                "# Run mdv with the specified arguments\n" +
                                $"mdv 0.dll '/g:{metaFile};{ilFile}' /stats+ /assemblyRefs+ /il+ /md+\n\n" +
                            ...
                            printfn "[HotReloadTest] Attempting to apply update for generation %d..." generationNumber
                            ...
                            if not MetadataUpdater.IsSupported then
                                ...
                                printfn "[HotReloadTest] Update applied successfully (generation %d)" generationNumber
                                ...
                                if newValue = expectedValue then
                                    printfn "[HotReloadTest] Hot reload success for generation %d! 🎉" generationNumber
                                    return true
                                else
                                    printfn "[HotReloadTest] Value didn't match expectation (%d vs expected %d)" newValue expectedValue
                                    return false
                    }

                let! gen1Success = applyDelta 43 43
                if not gen1Success then return ()

                let! gen2Success = applyDelta 44 44
                if not gen2Success then return ()
            } 
